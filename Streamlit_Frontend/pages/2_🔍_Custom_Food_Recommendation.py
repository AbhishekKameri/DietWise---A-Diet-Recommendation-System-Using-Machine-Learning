    ...
    def display_overview(self,recommendations):
        if recommendations!=None:
            ...
            with col2:
                ...
                # Show a dropdown only if the structure is correct
                if recommendations and isinstance(recommendations[0], dict):
                    recipe_names = [recipe['Name'] for recipe in recommendations if 'Name' in recipe]
                    selected_recipe_name = st.selectbox("Select a recipe", recipe_names)
                else:
                    st.warning("⚠️ The recommendations format is incorrect. Please check backend response.")
